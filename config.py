...
Optimized for Sevalla/Railway/Render deployment.
"""
...
def resolve_default_db_path(default_path: str) -> str:
    """Select a safe database path for Sevalla/Railway or local runs.

    Preference order:
    1) /var/lib/data (Common persistent storage path)
    2) /data (Railway persistent disk if mounted)
    3) /app/data (Some platforms use this)
    4) /tmp (ephemeral but writable)
    5) Provided default relative path
    """
    try:
        # Try common persistent storage paths
...
class Config:
    ...
    port: int = int(os.getenv('PORT', '8080'))  # Sevalla sets PORT automatically
...
async def health_check(request):
    ...
    return web.Response(text="OK - CNCF Issue Tracker Running", status=200)

async def start_health_server(port: int):
    """Start health check HTTP server for Sevalla/hosting platform."""
    app = web.Application()
...
class CNCFIssueTracker:
    ...
    async def check_all_repositories(self):
        ...
        new_issues_count = 0
        check_minutes = max(5, int(self.config.check_interval / 60) + self.config.check_buffer_minutes)
...
Bot is now monitoring for new issues! 🎯"""
    ...
    async def run(self):
        ...
        # Start health check server (required for Sevalla)
        try:
            await start_health_server(self.config.port)
        except Exception as e:
        ...
        if not startup_success:
            ...
            # Continue anyway - health check is running
